...
// for m>=n.
void Stack::jacobian_forward_openmp(Real* jacobian_out, bool row_major) {
  ...
  int n_block =
      (n_independent() + ADEPT_MULTIPASS_SIZE - 1) / ADEPT_MULTIPASS_SIZE;
...
#pragma omp parallel
  {
    std::vector<Block<ADEPT_MULTIPASS_SIZE, Real> > gradient_multipass_b(
        max_gradient_);
    ...
    for (iblock = 0; iblock < n_block; iblock++) {
      ...
      for (Offset idep = 0; idep < n_dependent(); idep++) {
        for (Offset i = 0; i < block_size; i++) {
          ...
          jacobian_out[idx] = gradient_multipass_b[dependent_offset_[idep]][i];
        }
      }
...
// for m<n.
void Stack::jacobian_reverse_openmp(Real* jacobian_out, bool row_major) {
  if (independent_offset_.empty() || dependent_offset_.empty()) {
    throw(dependents_or_independents_not_identified());
  }
  ...
  // block containing fewer than ADEPT_MULTIPASS_SIZE variables
  int n_block =
      (n_dependent() + ADEPT_MULTIPASS_SIZE - 1) / ADEPT_MULTIPASS_SIZE;
        ...
        if (n_non_zero) {
          ...
          for (Offset iop = statement_[ist - 1].end_plus_one;
            ...
            for (Offset i = 0; i < n_non_zero; i++) {
              gradient_multipass[i_non_zero[i]] +=
                  multiplier * a[i_non_zero[i]];
            }
#else
