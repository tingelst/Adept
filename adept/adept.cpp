...
void Stack::initialize(Offset n) {
...
void Stack::grow_operation_stack(Offset min) {
...
void Stack::grow_statement_stack(Offset min) {
  ...
  std::memcpy(new_statement, statement_, n_statements_ * sizeof(Statement));
  delete[] statement_;
...
// for m>=n.
void Stack::jacobian_forward(Real* jacobian_out, bool row_major) {
  ...
  for (Offset iblock = 0; iblock < n_block; iblock++) {
    ...
    for (Offset idep = 0; idep < n_dependent(); idep++) {
      for (Offset i = 0; i < ADEPT_MULTIPASS_SIZE; i++) {
        Offset row = idep;
        Offset col = i_independent + i;
        if (row_major) {
          idx = row * n_independent() + col;
        } else {
          idx = col * n_dependent() + row;
        }
        jacobian_out[idx] = gradient_multipass_[dependent_offset_[idep]][i];
      }
    }
...
// for m<n.
void Stack::jacobian_reverse(Real* jacobian_out, bool row_major) {
  if (independent_offset_.empty() || dependent_offset_.empty()) {
    throw(dependents_or_independents_not_identified());
  }
#ifdef _OPENMP
  ...
  for (Offset iblock = 0; iblock < n_block; iblock++) {
    ...
    for (Offset iindep = 0; iindep < n_independent(); iindep++) {
      for (Offset i = 0; i < ADEPT_MULTIPASS_SIZE; i++) {
        ...
        jacobian_out[idx] = gradient_multipass_[independent_offset_[iindep]][i];
      }
    }
...
void Stack::unregister_gradient_not_top(const Offset& gradient_offset) {
  enum { ADDED_AT_BASE, ADDED_AT_TOP, NEW_GAP, NOT_FOUND } status = NOT_FOUND;
  ...
  if (status == NOT_FOUND) {
    // Search other gaps
    for (GapListIterator it = gap_list_.begin(); it != gap_list_.end(); it++) {
      if (gradient_offset <= it->end + 1) {
        ...
        if (gradient_offset == it->start - 1) {
          ...
          most_recent_gap_ = gap_list_.insert(it, Gap(gradient_offset));
          status = NEW_GAP;
        }
        break;
      }
...
void Stack::jacobian(Real* jacobian_out, bool row_major) {
...
void Stack::print_gaps(std::ostream& os) const {
  for (GapList::const_iterator it = gap_list_.begin(); it != gap_list_.end();
       it++) {
    os << it->start << "-" << it->end << " ";
  }
}
...
void Stack::print_status(std::ostream& os) const {
  ...
  os << " and " << n_operations() << " operations (" << n_allocated_operations()
     << " allocated)\n";
  os << "      " << n_gradients_registered()
     << " gradients currently registered ";
